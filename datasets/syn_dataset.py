...
class synDataset(torch.utils.data.Dataset):
    def __init__(self, dataset: torch.utils.data.Dataset=None, n_samples=140, n_generation=14000, path='./data/syn_data_cpu.pt', load=True):
    ...
    def __getitem__(self, idx):
        ...
        if self.load == False:
            ...
            return self.masks[idx].unsqueeze(0), self.masks[idx].unsqueeze(0), self.keypoints[idx], self.poses[idx], self.bones[idx]
    
    def pregen_dataset(self):
        masks = torch.zeros((len(self.bones) * 100, 256, 256)).to(self.device)
        keyps = torch.zeros((len(self.bones) * 100, 36)).to(self.device)
        ps = torch.zeros((len(self.bones) * 100, 153)).to(self.device)
        bs = torch.zeros((len(self.bones) * 100, 24)).to(self.device)
        for i in tqdm(range(len(self.bones))):
            for j in range(100):
                bone_idx = i
                pose_idx = (i * 100) + j
                p_est = self.poses[pose_idx][None,:]
                b_est = (self.bones[bone_idx] + np.random.normal(loc=0, size=(24), scale=0.2))[None,:]
                ...
                masks[pose_idx] = mask
                keyps[pose_idx] = proj_kps
                ps[pose_idx] = p_est.squeeze()
                bs[pose_idx] = b_est.squeeze()
